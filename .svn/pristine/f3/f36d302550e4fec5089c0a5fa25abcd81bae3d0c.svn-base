...
Page({
  ...
  data: {
    ...
    selectb: ['性质', '国立', '公立', '私立'],
    selectc: ['专业', 'コンピュータ/Web/ゲーム', '機械/電気/電子', '自動車/航空技術', '建築/土木/インテリア/環境/バイオ', '看護/医療リハビリ', '福祉/保育/教育', '旅行/観光/ホテル', '留学/進学', 'ビジネス/経営', '音楽/イベント','マンガ/美術/工芸'],
    index: 0,
    indexa: 0,
    indexb: 0,
    indexc: 0,
  ...
  findSchool: function(event) {
    let that = this;
    that.data.schoolName = event.detail.value;
    
    wx.request({
      url: app.data.HOST + 'v1/schoolType?school_type_id=2&school_name=' + that.data.schoolName + '&school_area=&school_address=&school_nature=&school_major=',
      method: 'GET',
  ...
  initAll: function() {
    let that = this;
    wx.request({
      url: app.data.HOST + "v1/schoolType?school_type_id=2&school_name=&school_area=&school_address=&school_nature=&school_major=",
      method: "get",
      success: res => {
        console.log(res.data);
        that.setData({
  ...
  searchSchool: function() {
    let that = this;
    wx.request({
      url: app.data.HOST + 'v1/schoolType?school_type_id=2&school_name=&school_area=' + that.data.school_area + '&school_address=' + that.data.school_address + '&school_nature=' + that.data.school_nature + '&school_major=' + that.data.school_major,
      method: 'get',
      success: function(res) {
